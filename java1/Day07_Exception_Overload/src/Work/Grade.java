...
public class Grade {
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			if (choice == 1) { // 1. 학생정보 추가 (이름, 나이, 영어, 수학)
				insert(student, sc);
	...
	static void sorting(String[][] student) {		// 원본데이터 보호 필요!
		String studentSort[][];
		int count = 0;
	...
	static int sumAll(String[][] student) {
		int total = 0;
		for (int i = 0; i < student.length; i++) {
	...
	static void sumOne(String[][] student, Scanner sc, int findIndex) {
		int sum = 0;
		int i = findIndex;
		if (i != -1) {
	...
	static void update(String[][] student, Scanner sc, int findIndex) {
		int i = findIndex;
		if (i != -1) {
	...
	static void search(String[][] student, Scanner sc, int findIndex) {
		int i = findIndex;
		if (i != -1) {
	...
	static void delete(String[][] student, Scanner sc, int findIndex) {
		int i = findIndex;
		student[i][0] = null;
		student[i][1] = null;
		student[i][2] = null;
	...
	static void insert(String[][] student, Scanner sc) {
		...
		int select;
		while (true) {
			System.out.println("추가하실 학생번호를 선택하세요(1~5) 6.종료");
			select = sc.nextInt() - 1;
			if (select>6) {
	...
	static int inputname(String[][] student, Scanner sc) {
